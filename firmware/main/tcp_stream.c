...
static void tcp_accept_task_fn(void *arg)
{
    ...
    while (s_running)
    {
         ...
         * Values are configurable via menuconfig:
         *   CONFIG_STREAMER_TCP_KEEPIDLE  — seconds before first probe
         *   CONFIG_STREAMER_TCP_KEEPINTVL — seconds between probes
         *   CONFIG_STREAMER_TCP_KEEPCNT   — failed probes = dead
         * Total detection time ≈ keepidle + keepcnt * keepintvl. */
#if CFG_TCP_KEEPALIVE_ENABLED
        int keepalive = 1;
        int keepidle = CFG_TCP_KEEPIDLE_SEC;
        int keepintvl = CFG_TCP_KEEPINTVL_SEC;
        int keepcnt = CFG_TCP_KEEPCNT;
        ...
        /* FIX (ESP-RECONNECT-LEAK): SO_LINGER with linger=0 to force RST
         * close instead of FIN. This skips TIME_WAIT entirely, preventing
         * heap exhaustion during rapid reconnect cycles.
         *
         * Without this, each closed TCP connection enters TIME_WAIT for
         * 60-120 seconds on ESP8266 lwIP, holding ~8KB of pbuf memory.
         * After 3-4 rapid reconnects (e.g., server stop+start cycles),
         * 24-32KB of heap is consumed by TIME_WAIT PCBs -> sendto ENOMEM,
         * send EAGAIN -> death spiral -> only a reboot clears it.
         *
         * Configurable via menuconfig (CONFIG_STREAMER_TCP_LINGER_ENABLED).
         * Default: ON (recommended for streaming audio — we don't care
         * about in-flight data when closing). */
#if CFG_TCP_LINGER_ENABLED
        struct linger ling;
        ling.l_onoff = 1;
        ling.l_linger = 0;
        setsockopt(new_sock, SOL_SOCKET, SO_LINGER, &ling, sizeof(ling));
#endif
